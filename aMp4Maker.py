...
- tool can only convert image sequences, cannot transcode movies files.
...
for this_seq in sys.argv[1:]:
    ...
    for basename in baseNamesList:
        ...
        print('fseq has a length of {}'.format(len(fseq)))
        if os.path.splitext(fseq[0])[1][1:] in movFormatsDict:
